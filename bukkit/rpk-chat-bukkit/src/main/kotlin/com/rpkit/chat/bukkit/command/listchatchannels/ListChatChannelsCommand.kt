        ...
        chatChannelService.chatChannels.forEach { chatChannel ->
            chatChannel.listeners.thenAccept { listeners ->
                ...
                val template = plugin.messages["listchatchannels-item", mapOf(
                    "color" to chatChannel.color.toString()
                )]
                ...
                while (matcher.find()) {
                    ...
                    if (matcher.group() == "\${channel}") {
                        ...
                        chatChannelComponent.hoverEvent = HoverEvent(HoverEvent.Action.SHOW_TEXT, ComponentBuilder().append("Click to talk in ${chatChannel.name.value}").create())
                        if (chatColor != null) {
                        ...
                        if (listeners.any { listenerMinecraftProfile ->
                            ...
                            muteComponent.hoverEvent = HoverEvent(HoverEvent.Action.SHOW_TEXT, ComponentBuilder().append("Click to mute ${chatChannel.name.value}").create())
                            if (chatColor != null) {
                            ...
                            unmuteComponent.hoverEvent = HoverEvent(HoverEvent.Action.SHOW_TEXT, ComponentBuilder().append("Click to unmute ${chatChannel.name.value}").create())
                            if (chatColor != null) {
                        ...
                        if (match.startsWith("${ChatColor.COLOR_CHAR}x") && match.length == 14) {
                            ...
                            val colorOrFormat = org.bukkit.ChatColor.getByChar(match.drop(1)[0])
