                                        ...
                                        if (!fieldFound) {
                                            if (line.length >= i + "\${interval}".length) {
                                                if (line.substring(i, i + "\${interval}".length) == "\${interval}") {
                                                    val textComponent =
                                                        TextComponent("${paymentGroup.interval.toMillis() / 1000} seconds")
                                                    if (chatColor != null) {
                                                        textComponent.color = chatColor.asBungee()
                                                    }
                                                    if (chatFormat != null) {
                                        ...
                                        if (!fieldFound) {
                                            if (line.length >= i + "\${last_payment_time}".length) {
                                                if (line.substring(
                                                    ...
                                                    val textComponent = TextComponent(
                                                        dateFormat.format(
                                                            paymentGroup.lastPaymentTime.atZone(
                                            ...
                                            if (line.length >= i + "\${edit(name)}".length) {
                                                if (line.substring(i, i + "\${edit(name)}".length) == "\${edit(name)}") {
                                                    ...
                                                    textComponent.clickEvent = ClickEvent(
                                                        ...
                                                        "/payment set name ${paymentGroup.name.value}"
                                                    )
                                            ...
                                            if (!editFound) {
                                                if (line.length >= i + "\${edit(amount)}".length) {
                                                    if (line.substring(
                                                        ...
                                                        textComponent.clickEvent = ClickEvent(
                                                            ClickEvent.Action.RUN_COMMAND,
                                                            "/payment set amount ${paymentGroup.name.value}"
                                                        )
                                                        textComponent.hoverEvent = HoverEvent(
                                                            HoverEvent.Action.SHOW_TEXT,
                                            ...
                                            if (!editFound) {
                                                if (line.length >= i + "\${edit(currency)}".length) {
                                                    if (line.substring(
                                                        ...
                                                        textComponent.clickEvent = ClickEvent(
                                                            ClickEvent.Action.RUN_COMMAND,
                                                            "/payment set currency ${paymentGroup.name.value}"
                                                        )
                                                        textComponent.hoverEvent = HoverEvent(
                                                            HoverEvent.Action.SHOW_TEXT,
                                            ...
                                            if (!editFound) {
                                                if (line.length >= i + "\${edit(interval)}".length) {
                                                    if (line.substring(
                                                        ...
                                                        textComponent.clickEvent = ClickEvent(
                                                            ClickEvent.Action.RUN_COMMAND,
                                                            "/payment set interval ${paymentGroup.name.value}"
                                                        )
                                                        textComponent.hoverEvent = HoverEvent(
                                                            HoverEvent.Action.SHOW_TEXT,
